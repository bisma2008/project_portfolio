...
const Dashboard = () => {
    ...
    const fetchBuketData = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BUKET}/getAllByAdmin/${idAdmin}`
        );
  ...
  const totalSales = buketData.reduce(
    (total, buket) => total + (buket.sold * buket.hargaBuket || 0),
    0
  );
  const totalBuketsSold = buketData.reduce(
    (total, buket) => total + (buket.sold || 0),
    0
  );
  const averageRating = (
    buketData.reduce((total, buket) => total + (buket.rating || 0), 0) /
    buketData.length
  ).toFixed(1);
                ...
                <tbody>
                  {buketData.map((buket) => (
                    <tr key={buket.id} className="border-b">
                      ...
                      <td className="py-3 px-4">
                        Rp {(buket.hargaBuket || 0).toLocaleString()}
                      </td>
                      <td className="py-3 px-4">
